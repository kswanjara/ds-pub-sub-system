...
public class PubSubAgent {
            ...
            switch (sc.nextInt()) {
                ...
                case 3:
                    ...
                    if (temporary_list.size() > 0) {
                        ...
                        System.out.println("Incorrect topic name. Topic name not in the list of topics that you are subscribed to.\nPlease Try again...");
                        subscribe_helper();
                    }
    ...
    public void subscribe(List<String> keyword) {
        ...
        List<Topic> topicList = recievedPacket.getTopicList();
        List<Topic> filteredTopic = topicList.stream().filter(p -> !Collections.disjoint(p.getKeywords(), keyword)).collect(Collectors.toList());
        ...
        List<Topic> temporary_list = filteredTopic.stream().filter(p -> p.getName().equals(finalTopicSelected)).collect(Collectors.toList());
        if (temporary_list.size() > 0) {
            Topic subscribeToThisTopic = temporary_list.get(0);
            Packet packet1 = new Packet(subscribeToThisTopic, null, PacketConstants.SubscriberDto.toString(), subscriberDto );
            Packet reply = connectToEventManager(packet1);
        } else {
            System.out.println("Incorrect topic name. Topic name not yet created. Try advertising that topic if you want to. \nPlease Try again...");
            subscribe_helper();
        }
    }
